/*
 * @Descripttion: Jwt Middleware
 * @Author: William Wu
 * @Date: 2022-05-21 16:59:40
 * @LastEditors: William Wu
 * @LastEditTime: 2022-05-21 18:34:51
 */
package JwtMiddleware

import (
	"VideoHubGo/utils/JsonUtils"
	"VideoHubGo/utils/LogUtils"
	"net/http"
	"os"
	"strings"
	"time"

	"github.com/gin-gonic/gin"
	"github.com/golang-jwt/jwt"
	"github.com/spf13/viper"
)

/**
 * @Descripttion:  Jwt模型 - Jwt Model
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:02
 */
type JwtModel struct {
	UserId     int    `json:"uid"`
	UserName   string `json:"username"`
	IsAdmin    int    `json:"isadmin"`
	IsUploader int    `json:"isuploader"`
	jwt.StandardClaims
}

/**
 * @Descripttion: 生成Token - Create Token
 * @Author: William Wu
 * @Date: 2022/05/21 下午 05:01
 * @Param: 用户UID - User UId
 * @Param: 用户昵称 - User Name
 * @Param: 是否为管理员 - Is Admin
 * @Param: 是否为上传者 - Is Uploader
 * @Return: Token JWT
 */
func CreateToken(userId int, userName string, isAdmin int, isUploader int) (string, error) {

	//读取配置文件 - Read The Configuration File
	path, err := os.Getwd()
	if err != nil {
		LogUtils.Logger(err.Error())
	}
	config := viper.New()
	config.AddConfigPath(path + "/configs")
	config.SetConfigName("config")
	config.SetConfigType("yaml")

	//尝试进行配置读取 - Try Reading Configuration
	if err := config.ReadInConfig(); err != nil {
		LogUtils.Logger(err.Error())
	}

	key := []byte(config.GetString("jwt.key"))
	issuer := config.GetString("jwt.issuer")
	subject := config.GetString("jwt.subject")
	expireTimeConfig := config.GetInt("jwt.expireTime")

	expireTime := time.Now().Add(time.Duration(expireTimeConfig) * time.Hour) //过期时间 - Expire Time
	nowTime := time.Now()                                                     //当前时间 - Now Time
	claims := JwtModel{
		UserId:     userId,
		UserName:   userName,
		IsAdmin:    isAdmin,
		IsUploader: isUploader,
		StandardClaims: jwt.StandardClaims{
			ExpiresAt: expireTime.Unix(), //过期时间戳 - Expire Time
			IssuedAt:  nowTime.Unix(),    //当前时间戳 - Now Time
			Issuer:    issuer,            //颁发者签名 - Issuer Signature
			Subject:   subject,           //签名主题   - Signature Theme
		},
	}
	tokenStruct := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	return tokenStruct.SignedString(key)
}

/**
 * @Descripttion: 验证Token - Check Token
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:03
 * @Param: token
 * @Return: JwtModel
 */
func CheckToken(token string) (*JwtModel, bool) {
	//读取配置文件 - Read The Configuration File
	path, err := os.Getwd()
	if err != nil {
		LogUtils.Logger(err.Error())
	}
	config := viper.New()
	config.AddConfigPath(path + "/configs")
	config.SetConfigName("config")
	config.SetConfigType("yaml")

	//尝试进行配置读取 - Try Reading Configuration
	if err := config.ReadInConfig(); err != nil {
		LogUtils.Logger(err.Error())
	}

	key := []byte(config.GetString("jwt.key"))

	tokenObj, _ := jwt.ParseWithClaims(token, &JwtModel{}, func(token *jwt.Token) (interface{}, error) {
		return key, nil
	})
	if key, _ := tokenObj.Claims.(*JwtModel); tokenObj.Valid {
		return key, true
	} else {
		return nil, false
	}
}

/**
 * @Descripttion: JWT中间件 - JWT Middleware
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:03
 * @Param: *gin
 * @Return: Middleware Controller
 */
func JwtMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		//从请求头中获取Token - Get Token From Request Header
		tokenStr := c.Request.Header.Get("Authorization")
		//用户不存在 - User Non Existent
		if tokenStr == "" {
			c.JSON(http.StatusOK, JsonUtils.JsonResult(203, "非法请求 - Illegal Request", ""))
			c.Abort() //阻止执行 - Stopping
			return
		}
		//Token格式错误 - Token Format Error
		tokenSlice := strings.SplitN(tokenStr, " ", 2)
		if len(tokenSlice) != 2 && tokenSlice[0] != "Bearer" {
			c.JSON(http.StatusOK, JsonUtils.JsonResult(203, "Token格式错误 - Token Format Error", ""))
			c.Abort() //阻止执行 - Stopping
			return
		}
		//验证Token - Check Token
		tokenStruck, ok := CheckToken(tokenSlice[1])
		if !ok {
			c.JSON(http.StatusOK, JsonUtils.JsonResult(203, "Token错误 - Token Error", ""))
			c.Abort() //阻止执行 - Stopping
			return
		}
		//Token超时 - Token Timeout
		if time.Now().Unix() > tokenStruck.ExpiresAt {
			c.JSON(http.StatusOK, JsonUtils.JsonResult(203, "Token过期 - Token Expired", ""))
			c.Abort() //阻止执行 - Stopping
			return
		}

		c.Set("uid", tokenStruck.UserId)
		c.Set("username", tokenStruck.UserName)
		c.Set("isadmin", tokenStruck.IsAdmin)
		c.Set("isuploader", tokenStruck.IsUploader)
		c.Next()
	}
}

/**
 * @Descripttion: 获取Token中的UID - Get Token UID
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:04
 * @Param: gin.Context
 * @Return: uid
 */
func GetTokenUID(c *gin.Context) (uid int) {
	//从请求头中获取Token - Get Token From Request Header
	tokenStr := c.Request.Header.Get("Authorization")
	tokenSlice := strings.SplitN(tokenStr, " ", 2)
	//验证Token - Check Token
	tokenStruck, ok := CheckToken(tokenSlice[1])
	if !ok {
		c.JSON(http.StatusOK, JsonUtils.JsonResult(203, "Token错误 - Token Error", ""))
		c.Abort() //阻止执行 - Stopping
		return
	}

	return tokenStruck.UserId
}

/**
 * @Descripttion: 获取Token中的Username - Get Token Username
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:05
 * @Param: gin.Context
 * @Return: username
 */
func GetTokenUsername(c *gin.Context) (username string) {
	//从请求头中获取Token - Get Token From Request Header
	tokenStr := c.Request.Header.Get("Authorization")
	tokenSlice := strings.SplitN(tokenStr, " ", 2)
	//验证Token - Check Token
	tokenStruck, ok := CheckToken(tokenSlice[1])
	if !ok {
		c.JSON(http.StatusOK, gin.H{"code": 203, "msg": "Token错误 - Token Error"})
		c.Abort() //阻止执行 - Stopping
		return
	}

	return tokenStruck.UserName
}

/**
 * @Descripttion: 获取Token中的IsAdmin - Get Token IsAdmin
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:05
 * @Param: gin.Context
 * @Return:  isadmin
 */
func GetTokenIsAdmin(c *gin.Context) (isadmin int) {
	//从请求头中获取Token - Get Token From Request Header
	tokenStr := c.Request.Header.Get("Authorization")
	tokenSlice := strings.SplitN(tokenStr, " ", 2)
	//验证Token - Check Token
	tokenStruck, ok := CheckToken(tokenSlice[1])
	if !ok {
		c.JSON(http.StatusOK, gin.H{"code": 203, "msg": "Token错误 - Token Error"})
		c.Abort() //阻止执行 - Stopping
		return
	}

	return tokenStruck.IsAdmin
}

/**
 * @Descripttion: 获取Token中的IsUploader - Get Token IsUploader
 * @Author: William Wu
 * @Date: 2022/05/23 下午 04:05
 * @Param: gin.Context
 * @Return:  isuploader
 */
func GetTokenIs(c *gin.Context) (isuploader int) {
	//从请求头中获取Token - Get Token From Request Header
	tokenStr := c.Request.Header.Get("Authorization")
	tokenSlice := strings.SplitN(tokenStr, " ", 2)
	//验证Token - Check Token
	tokenStruck, ok := CheckToken(tokenSlice[1])
	if !ok {
		c.JSON(http.StatusOK, gin.H{"code": 203, "msg": "Token错误 - Token Error"})
		c.Abort() //阻止执行 - Stopping
		return
	}

	return tokenStruck.IsUploader
}
